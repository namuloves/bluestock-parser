...
const scrapeMadewell = async (url) => {
  console.log('👖 Starting Madewell scraper for:', url);
  ...
  try {
    ...
    // HTML fallbacks - Madewell specific
    if (!product.name) {
      product.name = $('h1.product-name').text().trim() ||
                     $('.pdp-main h1').text().trim() ||
                     $('meta[property="og:title"]').attr('content') || '';
    }
    
    if (!product.price) {
      const priceText = $('.product-price .price-sales').text().trim() ||
    ...
    const originalPriceText = $('.price-standard').text().trim();
    ...
    // Sizes
    $('.size-selector button').each((i, el) => {
    ...
    // Colors
    $('.color-selector button').each((i, el) => {
    ...
    console.log('✅ Successfully scraped Madewell product:', product.name || 'Unknown');
    return product;
    
  } catch (error) {
    console.error('❌ Madewell scraping error:', error.message);
    return { url, error: error.message, brand: 'Madewell' };
  }
};

module.exports = { scrapeMadewell };